package com.sulake.core.window.utils
{
   import com.sulake.core.window.enum.WindowParam;
   import flash.utils.Dictionary;
   
   public class ParamCodeTable extends WindowParam
   {
       
      
      public function ParamCodeTable()
      {
         super();
      }
      
      public static function fillTables(param1:Dictionary, param2:Dictionary = null) : void
      {
         var _loc3_:* = null;
         param1["MouseCursorType"] = const_113;
         param1["bound_to_parent_rect"] = const_65;
         param1["child_window"] = const_785;
         param1["embedded_controller"] = const_274;
         param1["resize_to_accommodate_children"] = const_43;
         param1["input_event_processor"] = const_32;
         param1["internal_event_handling"] = const_584;
         param1["mouse_dragging_target"] = const_200;
         param1["mouse_dragging_trigger"] = const_291;
         param1["mouse_scaling_target"] = const_249;
         param1["mouse_scaling_trigger"] = const_445;
         param1["horizontal_mouse_scaling_trigger"] = const_201;
         param1["vertical_mouse_scaling_trigger"] = const_172;
         param1["observe_parent_input_events"] = const_872;
         param1["optimize_region_to_layout_size"] = const_350;
         param1["parent_window"] = const_984;
         param1["relative_horizontal_scale_center"] = const_152;
         param1["relative_horizontal_scale_fixed"] = const_99;
         param1["relative_horizontal_scale_move"] = const_306;
         param1["relative_horizontal_scale_strech"] = const_213;
         param1["relative_scale_center"] = const_812;
         param1["relative_scale_fixed"] = const_672;
         param1["relative_scale_move"] = const_877;
         param1["relative_scale_strech"] = const_823;
         param1["relative_vertical_scale_center"] = const_150;
         param1["relative_vertical_scale_fixed"] = const_93;
         param1["relative_vertical_scale_move"] = const_271;
         param1["relative_vertical_scale_strech"] = const_245;
         param1["on_resize_align_left"] = const_609;
         param1["on_resize_align_right"] = const_412;
         param1["on_resize_align_center"] = const_340;
         param1["on_resize_align_top"] = const_673;
         param1["on_resize_align_bottom"] = const_437;
         param1["on_resize_align_middle"] = const_417;
         param1["on_accommodate_align_left"] = const_884;
         param1["on_accommodate_align_right"] = const_353;
         param1["on_accommodate_align_center"] = const_504;
         param1["on_accommodate_align_top"] = const_980;
         param1["on_accommodate_align_bottom"] = WINDOW_PARAM_ON_ACCOMMODATE_ALIGN_BOTTOM;
         param1["on_accommodate_align_middle"] = const_708;
         param1["route_input_events_to_parent"] = const_399;
         param1["use_parent_graphic_context"] = const_14;
         param1["draggable_with_mouse"] = const_858;
         param1["scalable_with_mouse"] = const_831;
         param1["reflect_horizontal_resize_to_parent"] = const_349;
         param1["reflect_vertical_resize_to_parent"] = WINDOW_PARAM_REFLECT_VERTICAL_RESIZE_TO_PARENT;
         param1["reflect_resize_to_parent"] = const_247;
         if(param2 != null)
         {
            for(_loc3_ in param1)
            {
               if(param2[param1[_loc3_]] != null)
               {
                  if(param1[_loc3_] != 0)
                  {
                     Logger.log("Conflictiong flags in window params: \"" + _loc3_ + "\"!");
                  }
               }
               param2[param1[_loc3_]] = _loc3_;
            }
         }
      }
   }
}
